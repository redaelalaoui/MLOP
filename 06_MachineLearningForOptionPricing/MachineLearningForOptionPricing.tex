...
\begin{frame}
    ...
    \item Due to the \textbf{curse of dimentionality}, we must reduce the features dimension.
...
\begin{itemize}
    ...
    \item The \textbf{asymptotic behavior} of the call in-the-money is \textbf{removed}. Learning the model will therefore be easier for the Gaussian process. 
...
\begin{itemize}
    \item Don't model the relation as one function, but as a distribution over functions.
...
\begin{itemize}
    \item Gaussian processes are fit to the data by optimizing the marginal probability of the data given the model with respect to the learned kernel hyperparameters.
    ...
    \end{align}
    Where $\theta$ is the hyperparameter vector, cointaining $\ell$ in the RBF case. 
...
\begin{itemize}
    ...
    \item Concerned about using a single $\ell$ when one dimension a range \textbf{1000 times} the other one. The \textbf{kernel will "care"} about the \textbf{bigger scales} as much as it does about the \textbf{smaller slaces}.
    \vspace{0.4cm}
    
    \item \textbf{Conclusion}: The data must me standardize so each dimension is on the same scale.
\end{itemize}
...
\textbf{Greedy feature selections}
    \begin{itemize}
...
\section{Gaussain process applied on a vanilla call option}
...
\item \textbf{Hypotheses}
    ...
    \begin{itemize}
        \item \textbf{Input}: Natural or Reduce variables.
...
\vspace{-1em}
\begin{itemize}
    \item \textbf{Dimension reduction} give better results.
    \item Use of a \textbf{bijective function} (price premium) reduce the MSE.
\end{itemize}
...
\begin{columns}
    \begin{column}{0.35 \textwidth}
        \textbf{Payoff}
        \begin{equation}
            \left(S_{T} - K\right)^{+}_{\left\{ \displaystyle\max_{0\leq t\leq T} S_t < B\right\}} \nonumber
        \end{equation}
    \end{column}
